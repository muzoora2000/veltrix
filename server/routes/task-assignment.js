...
router.post('/auto-assign', requireRole('national_admin', 'district_officer'), async (req, res) => {
  ...
  const officer = await db.prepare(`SELECT id, name, role FROM users WHERE district = ? AND role IN ('district_officer','technician','health_officer') AND active = 1 ORDER BY last_login DESC LIMIT 1`).get(district);
  ...
  const result = await db.prepare(`INSERT INTO task_assignments (report_id, incident_id, assigned_to, assigned_by, task_type, priority, status, department, district, description, location) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(
  ...
  await db.prepare(`INSERT INTO response_tickets (report_id, incident_id, ticket_number, title, description, priority, status, assigned_team, district, location, created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(
