...
app.use(helmet({
  crossOriginResourcePolicy: false // Keep allow resource loading globally (useful for media paths)
}));
...
// Lock down CORS mapping
app.use(cors({
  origin: function (origin, callback) {
    // Allow configured origins, no origin (e.g., mobile apps, curl), or local network testing
    if (!origin || CONFIG.corsOrigins.includes(origin) || origin.startsWith('http://192.168.')) {
      callback(null, true);
    } else {
